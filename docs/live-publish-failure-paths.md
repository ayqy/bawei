...
- 稳定成功经验：
  ...
  - 当前更稳的飞书策略仍是历史成功路径：**标题区单独处理，正文区走无图纯文本 token 写入**，最后再补 `原文链接`；不要把飞书跟其它渠道强行统一成 `raw/html + 图片` 链路。
- 当前状态：
  - 2026-03-29 用用户真实 `Default` profile 克隆回归后，仍落到 `https://accounts.feishu.cn/.../login`；说明 `.feishu.cn` cookie 存在，也不等于云文档目录仍可直接进入。
  - 已补 API 超时、同域导航重入和无图正文策略；2026-03-31 又补了一次正文 root 精确选择与历史成功路径回退，避免正文误写到标题区。
