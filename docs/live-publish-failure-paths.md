...
- 匿名公开 `6/10`：博客园、OSCHINA、墨问、少数派、腾讯云开发者社区、飞书文档；六个详情页均通过无 Cookie 标题、正文锚点和三张平台托管图片验收。
- 待审核 `2/10`：CSDN 文章 `163823287` 与今日头条 `pgc_id=7674937095891075636`。CSDN 详情页出现安全滑块，今日头条接口明确返回“审核中，无法修改”；二者都保留 `pending_review`，只复核状态，禁止再次投稿。
- 已退回 `1/10`：人人都是产品经理明确返回“作品与平台定位不符”，保留 `rejected` 和退回原因，同稿禁止再次投稿。
- 待人工验证 `1/10`：百家号接口返回 `errno=10000017`，要求验证绑定手机号。标题、正文、三张图片和封面均已填写并保留，但发布按钮只点击一次；自动化不得点击“去验证/其他验证”绕过，也不得重复提交。
- 技术失败 `0/10`、未登录 `0/10`。再次执行同稿时，六个公开渠道防重跳过，两个待审渠道只复核，退回与待验证渠道均保持冻结，没有产生重复稿件。
...
- 当前状态：
  - 已通过插件一键完整发布并验收通过。
  - 2026-08-17 同稿文章 `163823287` 已被平台接收但详情页仍有安全滑块，当前保持 `pending_review` 并禁止重投。
...
- 当前状态：
  ...
  - 已补登录页判定与同域导航重入。2026-08-17 同稿 `pgc_id=7674937095891075636` 明确处于审核中，保持 `pending_review` 并禁止重投。
